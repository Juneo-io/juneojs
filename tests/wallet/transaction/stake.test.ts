...
describe("StakeManager", (): void => {
  ...
  test("Should make a delegate transaction with amount less than min stake", async () => {
    ...
    await expect(mcnAccount.execute(summary)).rejects.toThrow(
      "amount 1 is less than min stake 10000000",
    );
  });
  ...
  test("Should make a delegate transaction with amount bigger than balance", async () => {
    ...
    const summary = await mcnAccount.estimate(delegateOperation);
    await expect(mcnAccount.execute(summary)).rejects.toThrow(
      "missing funds to perform operation: Delegate",
    );
  });
  ...
  test("Should make a delegate transaction with wrong nodeId", async () => {
    ...
    await expect(mcnAccount.estimate(delegateOperation)).rejects.toThrow(
      "value checksum is not valid",
    );
  });
  ...
  test("Should make a validate transaction with wrong nodeId", async () => {
    ...
    await expect(mcnAccount.estimate(validateOperation)).rejects.toThrow(
      "value is not base58",
    );
  });
